...
export const SignIn: React.FC = () => {
  ...
  const redirectPath = (location.state as any)?.from?.pathname || '/upload';
  ...
  const handleDemoLogin = () => {
    login('vaishnavidhamodharan2004@gmail.com', 'demo1234');
    navigate(redirectPath, { replace: true });
  };
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto select-none pt-4 pb-12">
      <div className="w-full p-8 rounded-3xl bg-[#FFF8ED] border border-[#6B315E]/20 shadow-[0_20px_50px_rgba(36,22,47,0.1)]">
        ...
        <h1 className="font-heading font-black text-2xl text-[#24162F] text-center mb-1">
          Welcome to DocuClean AI
        </h1>
        <p className="text-xs text-[#6F6670] text-center mb-6">
          Sign in to manage and restore your documents with zero data retention
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="secondary-btn w-full h-10 rounded-xl text-xs font-bold flex items-center justify-center gap-2 cursor-pointer mt-2"
          >
            <ShieldCheck className="w-4 h-4 text-[#3C8D87]" />
            <span>Instant Demo Access (Vaishnavi)</span>
          </button>
        </form>
        ...
        <div className="mt-6 pt-6 border-t border-[#EADCC8] text-center">
          <p className="text-xs text-[#6F6670]">
            ...
            <Link to="/signup" className="text-[#6B315E] font-bold hover:underline">
              Create one now
            </Link>
          </p>
        </div>
